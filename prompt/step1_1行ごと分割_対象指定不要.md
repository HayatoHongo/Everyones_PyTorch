```
promptフォルダについて、いくつかのstepがある。

このstepの順番通りに、notebookの編集を行う。

確実に1つのstepが完全に完了してから、次のstepに移動する。

1つずつ進めることを意識すること。

極めて重大な不足があれば停止してユーザーに支援を求めること。

重大な不足がなければ、最後のstepまで、柔軟に判断して自律的に進めること。
```
...
- class/defであれば、~クラス/関数を定義する、if/elseであれば もしも ~ の場合　のように、わかりやすい表現を用いる。
...
7) class 文については、クラス名, 関数名をmarkdownの見出しに変換して、インデントを発生させない。initについてはselfを使わずに、インスタンスとして設定しながら、1行ごとに分ける。その後の関数についても同様。
8) 既存のコメントアウトは対応するコードと同じセルに保持します。コメントアウトを消したり、独立したセルにはしないこと。
