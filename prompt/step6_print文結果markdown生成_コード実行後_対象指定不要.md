...
# この時点でコードが実行されていない場合、まずはコードを実行して、ファイルを保存する。

print 文が含まれるセルについて、その正しい結果を記載する。
なお、すでにファイルはコード実行済みであるため、出力をそのまま参照すること。特に環境を作成して実行する必要はない。
ただし、極端に長い出力がないかをチェックする。極端に長い出力については全部を読み込まずに、最後の10行だけを参照して、さらにそこから最も大事な数行だけについて結果を記載すればいい。また、極端に大きなテンソルの出力も全部を読み込まずに、サイズだけ確認すれば良い。
